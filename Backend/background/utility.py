...
def load_face_model():
  ...
  face_model.load_state_dict(torch.load("faceRecognition/Pytorch_ArcFace2/Model.pt",map_location='cpu'))
  return face_model

def load_antispoof_model():
  ANTI_SPOOF_MODEL_PATH = "antispoof/A.model"
  model = antiSpoofingInit(MODEL_PATH=ANTI_SPOOF_MODEL_PATH)   
  return model

def load_gait_model():
  chkpnt = torch.load("gait/sia_mnist_new.pth",map_location='cpu')
  model_ft = SiaCNN()
